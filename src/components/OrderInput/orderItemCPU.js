...
let timerT = { T: 0 };
const CurrentStakeFor = styled.div`
  text-align: right;
  font-size: 12px;
  margin-bottom: 10px;
  color: #5d97b6;
...
class OrderItem extends React.Component {
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    if (isChanged || isSymbolChange || priceModeChanged) {
      ...
      let propsPrice = U.calc(`${price} * 100`);
      let _priceValue = this.state.priceValue / 100 || propsPrice;
      const LIMIT = priceMode === 'LIMIT';
      const BUY = type === 'buy';
      let maxTargetAsset = this.getMaxTargetAsset(nextProps, price || '');
  ...
  getLabelMap = baseCurrency => {
    ...
    const { partition } = symbol;
    return {
      buy: {
        label1:
          partition === 'CPU' ? (
            <M id="orderInput.rate" />
          ) : (
            <M id="orderInput.buyPrice" />
          ),
        label2:
          partition === 'CPU' ? (
            <M id="orderInput.qty" />
          ) : (
            <M id="orderInput.sellQty" />
          ),
        label3: <M id="orderInput.buyRent" values={{ sth: 'EOS' }} />,
      ...
      sell: {
        ...
        label3: <M id="orderInput.sellRent" values={{ sth: 'EOS' }} />,
        label4: <M id="orderInput.needAuth" />,
        label5: <M id="orderInput.login" />,
        label6: <M id="orderInput.activeDevice" />,
        label7: <M id="orderInput.bindDevice" />,
  ...
  getMaxSlide = (props, maxTargetAsset) => {
    ...
    return { baseUnit, max: `${max}EOS`, maxRaw: max };
  };
  calcNextState = props => {
    const {
      asset,
      ...
      this.props.submitDelegate(
        {
          ...
          price: priceMode === 'LIMIT' ? U.calc(`${priceValue} / 100`) : '0',
        },
        () => {
  ...
  onpriceValueChange = _priceValue => {
    ...
    _priceValue = U.formatInsertData(_priceValue, 2);
    let maxTargetAsset = this.getMaxTargetAsset(this.props, _priceValue / 100);
    const { max, baseUnit, maxRaw } = this.getMaxSlide(
      this.props,
      maxTargetAsset
    );
  ...
  onInputChange = (value = 0) => {
    const {
      symbol: { lotSize, maxQty, minQty, quotePrecision },
      priceMode,
      type,
    } = this.props;
    this.props.clearOrderStateClickType();
  ...
  render() {
    ...
    const execR = ((inputValue * priceValue) / 100 || 0).toFixed(
      step1.length - 2
    );
        ...
        <div className="form-item">
          ...
          <div className="input-wrap">
            ...
            >
              ≈
              {(priceValue &&
                (((365 * 24) / duration) * priceValue).toFixed(2)) ||
                '--'}
              % ({formatMessage({ id: 'orderInput.yearPercent' })})
            </span>
          </div>
        </div>
        <div className="form-item">
        ...
        <div className={'button ' + ((loading && 'opacity-btn') || '')}>
          ...
          >
            ...
            {(isCustomer && label5) ||
              ...
              (needAuth && label4) ||
              (+maxAsset === 0 && BUY && (
                <M id="trade.goRechange" values={{ currency: 'EOS' }} />
              )) ||
              label3}
          </Button>
        </div>

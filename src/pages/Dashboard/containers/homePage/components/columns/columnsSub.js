...
const columnsSubFunc = (
  ...
  let columns = [
    ...
    },
    {
      ...
      width: 190,
      render: (v, i) => {
        const { duration, baseCurrency, quoteCurrency, partition } = i;
        if (partition === 'CPU') {
          return (
            <a
              onClick={() => {
    ...
    },
    {
      ...
      className: 'column-right',
      render: (v, i) => {
        const { baseCurrency, quoteCurrency, partition } = i;
        return (
          <a
            onClick={() => {
              urlJump(`/trade/${baseCurrency}_${quoteCurrency}`);
          ...
          >
            {(partition === 'CPU' && U.getPercentFormat(v)) || v}
          </a>
        );
      },
    },

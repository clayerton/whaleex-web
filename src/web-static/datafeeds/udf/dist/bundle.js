...
var HistoryProvider = /** @class */ (function() {
  ...
  HistoryProvider.prototype.getBars = function(
    ...
    var _this = this;
		// console.log('111',new Date(rangeStartDate*1000),new Date(rangeEndDate*1000))
		if(rangeEndDate*1000 > Date.now()) rangeEndDate = parseInt(Date.now()/1000);
    var startTime = parseInt(rangeStartDate / 5) * 5000;
    var endTime = parseInt(rangeEndDate / 5) * 5000;
		// console.log('222',new Date(startTime),new Date(endTime))
    var requestParams = {
    ...
    return new Promise(function(resolve, reject) {
      ...
      _this._requester
        ...
        .sendRequest(_this._datafeedUrl, "kline", requestParams)
        .then(function(response) {
					var isCPU=symbolInfo.walpartition==='CPU'
          window.klineLastData = response[0];
          var bars = [];
          ...
          if (!response.length) {
            ...
            for (var i = response.length - 1; i >= 0; --i) {
              var ref = response[i];
							...
							if(isCPU){
								ref.avgPrice=(ref.avgPrice*100).toFixed(2);
								ref.close=(ref.close*100).toFixed(2);
								ref.end=(ref.end*100).toFixed(2);
								ref.high=(ref.high*100).toFixed(2);
								ref.low=(ref.low*100).toFixed(2);
								ref.open=(ref.open*100).toFixed(2);
							}
              var start = ref.start;
              var close = ref.close;
              var open = ref.open;
...
var SymbolsStorage = /** @class */ (function () {
    function SymbolsStorage(datafeedUrl, datafeedSupportedResolutions, requester) {
      console.log('SymbolsStorage',datafeedUrl);
    ...
    SymbolsStorage.prototype.searchSymbols = function (searchString, exchange, symbolType, maxSearchResults) {
      console.log('searchSymbols',searchString,exchange);
    ...
    SymbolsStorage.prototype._requestExchangeData = function (exchange) {
      console.log('_requestExchangeData',exchange);
    ...
    SymbolsStorage.prototype._onExchangeDataReceived = function (exchange, data) {
      ...
      console.log('_onExchangeDataReceived',exchange,data);
        var symbolIndex = 0;
        try {
...
var UDFCompatibleDatafeedBase = /** @class */ (function() {
  ...
  UDFCompatibleDatafeedBase.prototype.onReady = function(callback) {
    ...
    this._configurationReadyPromise.then(function() {
      console.log('onReady', _this._configuration);
  ...
  UDFCompatibleDatafeedBase.prototype.getMarks = function(
    ...
    this._send('marks', requestParams)
      .then(function(response) {
        console.log('getMarks', response, resolution);
  ...
  UDFCompatibleDatafeedBase.prototype._sendUrl = function(
    baseUrl,
    urlPath,
    params
  ) {
    return this._requester.sendRequest(baseUrl, urlPath, params);
  };
...
var Requester = /** @class */ (function() {
  ...
  Requester.prototype.sendRequest = function(datafeedUrl, urlPath, params) {
    ...
    var api = datafeedUrl + ((urlPath && '/' + urlPath) || '');
    var type = urlPath;
    var urlParam = '';
    if (params !== undefined) {
    ...
    // console.log(api ,urlParam);
    logMessage('New request: ' + api + urlParam);
    // Send user cookies if the URL is on the same origin as the calling script.
    var options = { credentials: 'same-origin' };
    if (this._headers !== undefined) {
      options.headers = this._headers;
    }
    return fetch(api + urlParam, options)
      .then(function(response) {
        return response.text();
      })
      .then(function(responseTest) {
        var r = JSON.parse(responseTest);

        if (type === 'symbol') {
          var symbolName = params.symbol;
          var rFilteObj = r.filter(function(i) {
            return i.name === symbolName;
          })[0];
          var name = rFilteObj.name;
          var partition = rFilteObj.partition;
          var precision = rFilteObj.precision;
          var id = rFilteObj.id;
          r = {
            description: name,
            'exchange-listed': '',
